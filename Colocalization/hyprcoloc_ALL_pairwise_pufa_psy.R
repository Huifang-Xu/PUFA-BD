...
write.table(x=final_res, file = paste(Pathway_hyprcoloc_out,"PUFA_psy_pairwise_hyprcoloc_res.txt",sep = ""),
            sep = "\t", na="", quote = FALSE,row.names = FALSE, col.names = FALSE, append = TRUE)

for (n_tem in c(seq.int(1,11))) {
  for (trait_pair_another_n in c(seq.int(12,length(Trait_list)))) {
    Trait_dat_ref=read.csv(paste(Pathway_PUFA,"UKB_",Trait_list[n_tem],".a1effect.munge.rmInDels.uniq.tsv.gz", sep=""),
    ...
    Trait_dat=read.csv(paste(Pathway_PUFA,"UKB_",Trait_list[n_tem],".a1effect.munge.rmInDels.uniq.tsv.gz", sep=""),
                       header=T, as.is=T,sep = "\t")
    # traits_name <- c(traits_name, Trait_list[n])
    ...
    Trait_dat=read.csv(paste(Pathway_Psy,Trait_list[trait_pair_another_n],".a1effect.munge.rmInDels.uniq.tsv.gz", sep=""),
                       header=T, as.is=T,sep = "\t")
    # traits_name <- c(traits_name, Trait_list[n])
    ...
    Trait_dat=read.csv(paste(Pathway_PUFA,"UKB_",Trait_list[n_tem],".a1effect.munge.rmInDels.uniq.tsv.gz", sep=""),
                       header=T, as.is=T,sep = "\t")
    traits_name <- c(traits_name, Trait_list[n_tem])
    ...
    Trait_dat=read.csv(paste(Pathway_Psy,Trait_list[trait_pair_another_n],".a1effect.munge.rmInDels.uniq.tsv.gz", sep=""),
                       header=T, as.is=T,sep = "\t")
    traits_name <- c(traits_name, Trait_list[trait_pair_another_n])
            ...
            if (length(hyprcoloc_res$results$candidate_snp) > 0 ) {
              ...
              if (length(hyprcoloc_res$results$candidate_snp) > 0 ) {
                if (is.na(hyprcoloc_res$results$candidate_snp[1]) == FALSE) {
                  ...
                  # print(res)
                  write.table(x=res, file = paste(Pathway_hyprcoloc_out,"PUFA_psy_pairwise_hyprcoloc_res.txt",sep = ""),
                              sep = "\t", na="", quote = FALSE,row.names = FALSE, col.names = FALSE, append = TRUE)
                }
              }
            }
...
res=read.csv(paste(Pathway_hyprcoloc_out,"PUFA_psy_pairwise_hyprcoloc_res.txt",sep = ""),
             header=T, as.is=T,sep = "\t")

res$pos_tem=as.integer(sapply(strsplit(res$Location,fixed = TRUE, split= "-"),"[",2))

res <- res[order(res$traits),]
res <- res[order(res$Chr,res$pos_tem),]

res = subset(res, select = -c(pos_tem))

write.table(x=res, file = paste(Pathway_hyprcoloc_out,"PUFA_psy_pairwise_hyprcoloc_res_final.txt",sep = ""),
            sep = "\t", na="", quote = FALSE,row.names = FALSE, col.names = TRUE, append = TRUE)
