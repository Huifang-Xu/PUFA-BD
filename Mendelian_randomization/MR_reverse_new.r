...
write.table(final_res, file= paste(Pathway_out,"All_MR_psy_vs_pufa.txt", sep=""), col.names = FALSE, append = TRUE,
...
for (n in c(1:length(met_sig_list$p1))) {
  exp_dat_file=paste(Pathway_Psy,met_sig_list$p1[n],".a1effect.munge.rmInDels.uniq.tsv.gz", sep="")
  ...
  exp_dat$id.exposure=met_sig_list$p1[n]
  exp_dat$exposure=met_sig_list$p1[n]
  ...
  if (length(exp_dat$SNP)>0) {
    ...
    if (length(exp_dat$SNP)>0) {
      ...
      outcomefile=paste(Pathway_PUFA,"UKB_",met_sig_list$p2[n],".a1effect.munge.rmInDels.uniq.tsv.gz", sep="")
      ####### Change csv file
      ...
      outcome_dat$id.outcome=met_sig_list$p2[n]
      outcome_dat$outcome=met_sig_list$p2[n]
      #exp_dat$SD=exp_dat1$SD
      outcome_dat$SD=1
